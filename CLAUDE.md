...
**Skills — vérifier et choisir le meilleur À CHAQUE tâche.** Il existe des skills pour **quantité de domaines — pas seulement le design** : interface, revue de code, tests, documents (Word/Excel/PDF/présentations), visualisation de données, sécurité, lancement de l'app, workflows git, configuration, et bien d'autres. Et la liste **change** d'une session à l'autre. Donc, **pour chaque tâche — quelle qu'elle soit — vérifie les skills réellement disponibles dans ta session et charge le(s) plus adapté(s)** ; ne te fie jamais à une liste de noms figée écrite ici. Réflexe par défaut : *avant de te lancer, regarde s'il existe un skill qui couvre ce que tu t'apprêtes à faire* — si oui, utilise-le ; si aucun ne colle, avance sans. Les garde-fous du projet, eux, se chargent automatiquement.
